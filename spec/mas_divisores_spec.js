var mas_divisores = require('../src/mas_divisores').mas_divisores;
var total_divisores = require('../src/mas_divisores').total_divisores;
var factores_primos = require('../src/mas_divisores').factores_primos;
var resumir = require('../src/mas_divisores').resumir;

describe("mas_divisores", function() {
  it("returns 1 with 1 divisor with max = 1", function() {
    var max = mas_divisores(1);
...
describe("total_divisores", function() {
  it("returns correct number of total divisors", function() {
    expect(total_divisores(10)).toEqual(4);
    expect(total_divisores(18)).toEqual(6);
    expect(total_divisores(24)).toEqual(8);
  });
});

describe("factores_primos", function() {
  it("returns an array with all prime factores", function() {
    expect(factores_primos(10).sort()).toEqual([2,5]);
    expect(factores_primos(18).sort()).toEqual([2,3,3]);
    expect(factores_primos(24).sort()).toEqual([2,2,2,3]);
  });
});

describe("resumir", function() {
  it("returns a resume counting objects from array", function() {
    expect(resumir([2,5])).toEqual([[2,1],[5,1]]);
    expect(resumir([2,2,2,3])).toEqual([[2,3],[3,1]]);
  });
});
